...
async function initializeCloudSettings() {
  ...
  try {
    ...
    const appliedKey = 'restart-admin-reset-applied';
    if (resetAt && localStorage.getItem(appliedKey) !== String(resetAt)) {
      window.RestartApp?.resetLocal?.();
      localStorage.removeItem('restart-square-position');
      localStorage.removeItem('restart-daily-v135');
      localStorage.setItem(appliedKey, String(resetAt));
      const user = await cloudCurrentUser();
      if (user) {
        const nextExtra = { ...(row.extra || {}), admin_full_reset_requested: false, admin_full_reset_applied_at: new Date().toISOString() };
        await cloud.from('settings').update({ extra: nextExtra }).eq('user_id', user.id);
      }
