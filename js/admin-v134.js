...
window.showAdminTab=function(tab){['players','feedback','chat'].forEach(x=>{$(`admin-${x}`)?.classList.toggle('hidden',x!==tab);$(`adminTab-${x}`)?.classList.toggle('active',x===tab)});if(tab==='players')loadAdminPlayers();else if(tab==='feedback')loadAdminFeedback();else loadAdminChat()};
...
window.loadAdminChat=async function(){const box=$('adminChatList');if(!box)return;box.innerHTML='<div class="feedback-empty">正在載入聊天…</div>';try{const rows=await rpc('admin_list_square_messages_v1315',{})||[];box.innerHTML=rows.map(r=>`<article class="feedback-record"><strong>${esc(r.display_name||r.email||'玩家')}</strong><small>${esc(new Date(r.created_at).toLocaleString('zh-TW'))}</small><p>${esc(r.message)}</p><button class="btn danger wide" onclick="deleteAdminChat('${esc(r.id)}')">刪除訊息</button></article>`).join('')||'<div class="feedback-empty">目前沒有聊天訊息</div>'}catch(e){box.innerHTML=`<div class="feedback-empty error">${esc(e.message)}</div>`}};
window.deleteAdminChat=async function(id){if(!confirm('確定刪除這則訊息？'))return;try{await rpc('admin_delete_square_message_v1315',{message_id:id});notify('訊息已刪除');loadAdminChat()}catch(e){notify('刪除失敗：'+e.message)}};
