...
(function () {
  ...
  async function renderCanvas(canvas) {
    ...
    const equippedTop=canvas.dataset.top||'',equippedBottom=canvas.dataset.bottom||'',equippedSet=canvas.dataset.set||'';
    const topSrc=equippedTop?(ASSETS.shopTopWhite[direction]||ASSETS.shopTopWhite.front):look.top;
    const bottomSrc=equippedBottom?(ASSETS.shopBottomBlack[direction]||ASSETS.shopBottomBlack.front):look.bottom;
    ...
    try {
      const [rawBase, top, bottom, shoes, rawFace, rawHair] = await Promise.all([
        loadImage(look.base), loadImage(topSrc), loadImage(bottomSrc), loadImage(look.shoes), loadImage(look.face), loadImage(look.hair)
      ]);
  ...
  function canvasMarkup(gender = 'male', size = 'full', equippedOverride = null) {
    ...
    return `<canvas class="avatar-canvas avatar-canvas-${size}" data-gender="${gender}" data-direction="${direction}" data-hair-color="${hairColor}" data-eye-color="${eyeColor}" data-skin-tone="${skinTone}" data-thumb-slot="${thumbSlot}" data-top="${eq.top||''}" data-bottom="${eq.bottom||''}" data-set="${eq.set||''}" aria-label="${gender === 'female' ? '女生' : '男生'}角色"></canvas>`;
  }
