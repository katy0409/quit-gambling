...
(function () {
  ...
  const ASSETS = {
    ...
    maleHair: `${ASSET_ROOT}/hair/male/male_hair_001_front.png`,
    femaleHair: `${ASSET_ROOT}/hair/female/female_hair_001_front.png`
  };

  const HEAD_THUMB_SLOTS = new Set(['hair','hairColor','eyeShape','eyeColor','expression']);

  /* New base files already use the common 1024 x 1024 artboard. */
  const TRANSFORM = {
    base: { scale: 1, x: 0, y: 0 },
    faceMale: { scale: 0.827, x: 88, y: 64 },
    faceFemale: { scale: 0.827, x: 88, y: 82 },
    hair: { scale: 0.827, x: 88, y: 26 }
  };
  ...
  function resolveLook(gender, direction) {
    ...
    return {
      ...
      hair: female ? ASSETS.femaleHair : ASSETS.maleHair
    };
  }
  ...
  function stage(viewport) {
    if (viewport.headCrop) {
      const virtualSize = 660;
      const fit = Math.max(viewport.cssWidth / virtualSize, viewport.cssHeight / virtualSize);
      return { fit, x: viewport.cssWidth / 2 - 512 * fit, y: viewport.cssHeight / 2 - 350 * fit };
    }
  ...
  async function renderCanvas(canvas) {
    ...
    try {
      const [base, face, rawHair] = await Promise.all([
        loadImage(look.base), loadImage(look.face), loadImage(look.hair)
      ]);
      const hair = await tintHairImage(rawHair, hairColor);
      ...
      drawArtboardLayer(ctx, face, viewport, gender === 'female' ? TRANSFORM.faceFemale : TRANSFORM.faceMale);
  ...
  function canvasMarkup(gender = 'male', size = 'full', equippedOverride = null) {
    ...
    return `<canvas class="avatar-canvas avatar-canvas-${size}" data-gender="${gender}" data-direction="${direction}" data-hair-color="${hairColor}" data-thumb-slot="${thumbSlot}" aria-label="${gender === 'female' ? '女生' : '男生'}角色"></canvas>`;
  }
  ...
  window.renderCharacterChoices = function () {
    ...
    const starterCard = char => `<button class="starter-character ${pending === char.id ? 'selected' : ''}" onclick="previewCharacter('${char.id}')"><span>${char.name}</span>${canvasMarkup(char.gender, 'select', { hairColor: 'hair-rgb-155-103-055' })}</button>`;
    const starters = Array.isArray(window.STARTER_CHARACTERS) ? window.STARTER_CHARACTERS : [];
    const males = starters.filter(char => char.gender === 'male');
    const females = starters.filter(char => char.gender === 'female');
    box.innerHTML = `
      <section class="starter-group"><h3>♂ 男生角色</h3><div class="starter-grid v131-starter-row">${males.map(starterCard).join('')}</div></section>
      <section class="starter-group"><h3>♀ 女生角色</h3><div class="starter-grid v131-starter-row">${females.map(starterCard).join('')}</div></section>`;
