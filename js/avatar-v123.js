...
(function () {
  ...
  const SKIN_COLORS={porcelain:'#fff4e8',fair:'#f5dcc8',natural:'#deb08d',wheat:'#b97852',deep:'#70452f'};
  ...
  async function renderCanvas(canvas) {
    ...
    try {
      const [rawBase, top, bottom, shoes, rawFace, rawHair] = await Promise.all([
        loadImage(look.base), loadImage(look.top), loadImage(look.bottom), loadImage(look.shoes), loadImage(look.face), loadImage(look.hair)
      ]);
      const [base,hair, face] = await Promise.all([tintSkinImage(rawBase,resolveSkinTone()),tintHairImage(rawHair, hairColor), tintEyeImage(rawFace, eyeColor)]);
      ...
      drawArtboardLayer(ctx, top, viewport, TRANSFORM.base);
