...
(function () {
  ...
  async function renderCanvas(canvas) {
    ...
    const equippedTop=canvas.dataset.top||'',equippedBottom=canvas.dataset.bottom||'',equippedSet=canvas.dataset.set||'',equippedShoes=canvas.dataset.shoes||'';
    ...
    try {
      const [rawBase, top, bottom, shoes, rawFace, rawHair] = await Promise.all([
        loadImage(look.base), loadImage(topSrc), loadImage(bottomSrc), equippedShoes ? loadImage(look.shoes) : Promise.resolve(null), loadImage(look.face), loadImage(look.hair)
      ]);
  ...
  function canvasMarkup(gender = 'male', size = 'full', equippedOverride = null) {
    ...
    return `<canvas class="avatar-canvas avatar-canvas-${size}" data-gender="${gender}" data-direction="${direction}" data-hair-color="${hairColor}" data-eye-color="${eyeColor}" data-skin-tone="${skinTone}" data-thumb-slot="${thumbSlot}" data-top="${eq.top||''}" data-bottom="${eq.bottom||''}" data-set="${eq.set||''}" data-shoes="${eq.shoes||''}" aria-label="${gender === 'female' ? '女生' : '男生'}角色"></canvas>`;
  }
