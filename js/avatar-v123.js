...
(function () {
  ...
  const ASSETS = {
    ...
    bodyLeft: `${ASSET_ROOT}/body/body_left.png`,
    bodyRight: `${ASSET_ROOT}/body/body_right.png`,
  ...
  */
  const TRANSFORM = {
    ...
    bodySide: { scale: 1, x: 0, y: 0 },
    head: { scale: 0.78, x: 112, y: 50 },
    face: { scale: 0.78, x: 112, y: 60 },
    headLeft: { scale: 0.78, x: 88, y: 50 },
    faceLeft: { scale: 0.78, x: 88, y: 60 },
    headRight: { scale: 0.78, x: 136, y: 50 },
    faceRight: { scale: 0.78, x: 136, y: 60 }
  };
  ...
  function resolveLook(gender, direction = 'center') {
    const female = gender === 'female';
    const body = direction === 'left' ? ASSETS.bodyLeft : direction === 'right' ? ASSETS.bodyRight : ASSETS.body;
    return {
      gender,
      body,
      head: ASSETS.head,
  ...
  async function renderCanvas(canvas) {
    ...
    const direction = canvas.dataset.direction || 'center';
    const crop = canvas.dataset.crop || 'full';
    ...
    try {
      ...
      const headTransform = direction === 'left' ? TRANSFORM.headLeft : direction === 'right' ? TRANSFORM.headRight : TRANSFORM.head;
      const faceTransform = direction === 'left' ? TRANSFORM.faceLeft : direction === 'right' ? TRANSFORM.faceRight : TRANSFORM.face;

      // Correct RPG layering order. Side body assets are already positioned on the 1024 artboard.
      drawArtboardLayer(ctx, hairBack, viewport, headTransform);
      drawArtboardLayer(ctx, body, viewport, side ? TRANSFORM.bodySide : TRANSFORM.body);
      drawArtboardLayer(ctx, head, viewport, headTransform);
      drawArtboardLayer(ctx, face, viewport, faceTransform);
      drawArtboardLayer(ctx, hairFront, viewport, headTransform);

      if (crop === 'head') {
        // Hair/face products use the same avatar render, enlarged and clipped to neck-up.
        const snapshot = document.createElement('canvas');
        snapshot.width = canvas.width; snapshot.height = canvas.height;
        snapshot.getContext('2d').drawImage(canvas, 0, 0);
        ctx.clearRect(0, 0, viewport.cssWidth, viewport.cssHeight);
        const sx = snapshot.width * 0.20, sy = snapshot.height * 0.04;
        const sw = snapshot.width * 0.60, sh = snapshot.height * 0.54;
        ctx.drawImage(snapshot, sx, sy, sw, sh, 0, 0, viewport.cssWidth, viewport.cssHeight);
      }
    } catch (error) {
      console.error(error);
      ctx.clearRect(0, 0, viewport.cssWidth, viewport.cssHeight);
  ...
  function canvasMarkup(gender = 'male', size = 'full', equippedOverride = null, crop = 'full') {
    const hairColor = resolveHairColor(equippedOverride);
    const direction = size === 'full' ? (state?.game?.avatarDirection || 'center') : 'center';
    return `<canvas class="avatar-canvas avatar-canvas-${size}" data-gender="${gender}" data-direction="${direction}" data-crop="${crop}" data-hair-color="${hairColor}" aria-label="${gender === 'female' ? '女生' : '男生'}角色"></canvas>`;
  }
  ...
  window.avatarSvg = function (size = 'full', equippedOverride = null, crop = 'full') {
    return canvasMarkup(currentGender(), size, equippedOverride, crop);
  };

  window.setAvatarDirection = function (direction) {
  ...
  window.renderCharacterChoices = function () {
    ...
    box.innerHTML = `
      <div class="v123-preview-note">
        <strong>V12.3.6 三方向角色測試</strong>
        <span>正面為完整角色；左右方向先套用你提供的側向身體，側向頭部素材之後再補。</span>
      </div>
