...
(function () {
  ...
  const DEFAULT_SKIN_COLOR = '#F7D7C4';
  const SKIN_PALETTE = { porcelain:'#FFF1E5', ivory:'#F7D7C4', natural:'#D9A27F', tan:'#A96F4F', deep:'#6F4433' };
  ...
  function resolveSkinColor(override) {
    const raw = override?.skinColor || override?.skin_color || (()=>{ try{return state?.game?.pendingSkinColor || state?.game?.skinColor || 'ivory'}catch(_){return 'ivory'} })();
    return SKIN_PALETTE[raw] || (/^#[0-9a-f]{6}$/i.test(String(raw||'')) ? String(raw) : DEFAULT_SKIN_COLOR);
  }

  function tintSkinImage(img, colorHex) {
    const key = `skin-tint:${img.src}|${colorHex}`;
    if (imageCache.has(key)) return imageCache.get(key);
    const promise = Promise.resolve().then(() => {
      const canvas=document.createElement('canvas'); canvas.width=ARTBOARD; canvas.height=ARTBOARD;
      const ctx=canvas.getContext('2d',{willReadFrequently:true}); ctx.drawImage(img,0,0,ARTBOARD,ARTBOARD);
      const imageData=ctx.getImageData(0,0,ARTBOARD,ARTBOARD), data=imageData.data;
      const target=hexToRgb(colorHex);
      for(let i=0;i<data.length;i+=4){
        if(data[i+3]===0)continue; const r=data[i],g=data[i+1],b=data[i+2];
        const isSkin=r>150 && r>=g*1.01 && g>b*1.02 && (r-b)>18;
        if(!isSkin)continue;
        const lum=(r+g+b)/765, shade=.58+lum*.55;
        data[i]=Math.min(255,target.r*shade); data[i+1]=Math.min(255,target.g*shade); data[i+2]=Math.min(255,target.b*shade);
      }
  ...
  function stage(viewport) {
    ...
    if (viewport.headCrop) {
      ...
      crop = { x: 270, y: 250, width: 484, height: 620 };
  ...
  async function renderCanvas(canvas) {
    ...
    const skinColor = resolveSkinColor({skinColor:canvas.dataset.skinColor});
    ...
    try {
      const [base, bottom, shoes, rawFace, rawHair] = await Promise.all([
        loadImage(look.base), loadImage(look.bottom), loadImage(look.shoes), loadImage(look.face), loadImage(look.hair)
      ]);
      const [skinBase, hair, face] = await Promise.all([tintSkinImage(base, skinColor), tintHairImage(rawHair, hairColor), tintEyeImage(rawFace, eyeColor)]);
      if (canvasState.get(canvas) !== token) return;
      ctx.clearRect(0, 0, viewport.cssWidth, viewport.cssHeight);

      drawArtboardLayer(ctx, skinBase, viewport, TRANSFORM.base);
      drawArtboardLayer(ctx, bottom, viewport, TRANSFORM.base);
      drawArtboardLayer(ctx, shoes, viewport, TRANSFORM.base);
  ...
  function canvasMarkup(gender = 'male', size = 'full', equippedOverride = null) {
    ...
    const skinColor = resolveSkinColor(equippedOverride);
    const direction = ['full', 'mini', 'square'].includes(size) ? currentDirection() : 'front';
    const thumbSlot = equippedOverride?.__thumbSlot || '';
    return `<canvas class="avatar-canvas avatar-canvas-${size}" data-gender="${gender}" data-direction="${direction}" data-hair-color="${hairColor}" data-eye-color="${eyeColor}" data-skin-color="${skinColor}" data-thumb-slot="${thumbSlot}" aria-label="${gender === 'female' ? '女生' : '男生'}角色"></canvas>`;
  }
  ...
  window.renderCharacterChoices = function () {
    ...
    if(typeof window.renderSkinColorChoices==='function') window.renderSkinColorChoices();
    box.innerHTML = ['male','female'].map(gender => `<section class="starter-group"><h3><span class="gender-symbol">${gender==='male'?'♂':'♀'}</span><span>${gender==='male'?'男生角色':'女生角色'}</span></h3><div class="starter-grid v131-starter-row">${starters.filter(x=>x.gender===gender).map(card).join('')}</div></section>`).join('');
