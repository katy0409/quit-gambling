/* Restart V12.3.2 layered avatar preview
...
(function(){
  ...
  renderCharacterChoices=function(){
    ...
    box.innerHTML=`
      <div class="v123-preview-note">
        <strong>V12.3 分層角色預覽</strong>
        <span>目前已接入：共用身體、頭型、男女初始表情、男生髮型 001 前髮與後髮。</span>
      </div>
  ...
  confirmCharacter=function(){
    ...
    if(state.game.character){
      state.game.avatarEngine='v12.3.2-layered-png';
      save();
      renderGame();
      renderGameHome();
    }
  ...
  // V12.3.2 interaction: wave instead of jump.
  animateGameAvatar=function(){
    const el=$('gameAvatar');
    if(!el)return;
    el.classList.remove('avatar-wave');
    void el.offsetWidth;
    el.classList.add('avatar-wave');
    window.setTimeout(()=>el.classList.remove('avatar-wave'),1100);
  };

  function refreshV123(){
    try{
      if(state&&state.game&&state.game.character){renderGameHome();}
    }catch(e){console.warn('V12.3 avatar refresh skipped',e)}
  }
