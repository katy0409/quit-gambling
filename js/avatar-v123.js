/* Restart V13.4 Avatar Engine
   Three-direction body preview, swipe direction switching, fixed body sizing,
...
(function () {
  ...
  const TRANSFORM = {
    body: { scale: 0.718, x: 142, y: 290 },
    head: { scale: 0.827, x: 88, y: 26 },
  ...
  async function renderCanvas(canvas) {
    ...
    try {
      ...
      if (!isRear) drawArtboardLayer(ctx, face, viewport, gender === 'female' ? TRANSFORM.faceFemale : TRANSFORM.faceMale);
  ...
  window.animateGameAvatar = function () {
    const el = $('gameAvatar');
    if (!el) return;
    if (currentDirection() !== 'front') setDirection('front', false);
    window.setTimeout(() => {
