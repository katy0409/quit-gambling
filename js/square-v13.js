(() => {
  ...
  function playerMarkup(player) {
    ...
    const playerDirection = ['front','right','backRight','back','backLeft','left'].includes(player.direction) ? player.direction : 'front';
  ...
  function applyCanvasColors(players) {
    ...
    elements.forEach((el, index) => {
      ...
      if (canvas && player) {
        ...
        canvas.dataset.direction = ['front','right','backRight','back','backLeft','left'].includes(player.direction) ? player.direction : 'front';
      }
    });
  ...
  function chooseDirection(dx, dy) {
    ...
    if (dy < 0 && ay > ax * .72) return 'back';
    if (dy < 0) return dx >= 0 ? 'backRight' : 'backLeft';
    if (dy > 0 && ay > ax * .9) return 'front';
  ...
  function moveFromPointer(event) {
    ...
    target.y = clamp(((event.clientY - rect.top) / rect.height) * 100, 28, 91);
