(() => {
  ...
  const MAP_W = 2458;
  const MAP_H = 1229;
  const STREET_MIN_Y = 76;
  const STREET_MAX_Y = 94;
  const START = { x: 48, y: 87 };
  let channel = null;
  let selfUser = null;
  let selfKey = '';
  let position = { ...START };
  ...
  function getLocalLook() {
    try {
      ...
      const equipped = { ...(state?.game?.equipped || {}) };
      const colorId = equipped.hairColor;
      const colorItem = typeof itemById === 'function' ? itemById(colorId) : null;
      const eyeId = equipped.eyeColor;
      const eyeItem = typeof itemById === 'function' ? itemById(eyeId) : null;
      return {
        gender,
        hairColor: colorItem?.color || '#9B6737',
        eyeColor: eyeItem?.color || '#6D462E',
        skinTone: state?.game?.skinTone || 'fair',
        top: equipped.top || '',
        bottom: equipped.bottom || '',
        set: equipped.set || '',
        shoes: equipped.shoes || '',
        expression: equipped.expression || 'expression-soft',
        hair: equipped.hair || ''
      };
    } catch (_) {
      return { gender: 'male', hairColor: '#9B6737', eyeColor:'#6D462E', skinTone:'fair', top:'', bottom:'', set:'', shoes:'', expression:'expression-soft', hair:'' };
    }
  }
  ...
  function presencePayload() {
    ...
    return {
      ...
      equipped_top: look.top,
      equipped_bottom: look.bottom,
      equipped_set: look.set,
      equipped_shoes: look.shoes,
      equipped_expression: look.expression,
      equipped_hair: look.hair,
  ...
  function playerMarkup(player) {
    ...
    return `<div class="square-player ${mine ? 'is-me' : ''}" data-user-id="${escapeHtml(player.user_id || player.presenceKey)}" style="left:${x}%;top:${y}%;z-index:${depth}">
      ...
      <div class="square-avatar-wrap">${typeof avatarV123Markup === 'function' ? avatarV123Markup(gender, 'square', equipped) : ''}</div>
  ...
  function applyCanvasColors(players) {
    ...
    elements.forEach((el, index) => {
      ...
      if (canvas && player) {
        ...
        canvas.dataset.skinTone = player.skin_tone || 'fair';
        canvas.dataset.top = player.equipped_top || '';
        canvas.dataset.bottom = player.equipped_bottom || '';
        canvas.dataset.set = player.equipped_set || '';
        canvas.dataset.shoes = player.equipped_shoes || '';
        canvas.dataset.expression = player.equipped_expression || 'expression-soft';
      }
    });
  ...
  async function connectSquare() {
    ...
    if (saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)) { position = {x:clamp(saved.x,5,95),y:clamp(saved.y,STREET_MIN_Y,STREET_MAX_Y)}; target = { ...position }; }
  ...
  // V13.25: wardrobe/shop changes reuse the same presence payload and canvas renderer,
  // so the square immediately reflects the latest equipped layers.
  window.refreshSquareAvatar = async function () {
    if (!joined || !channel) return;
    try {
      await syncTrack();
      renderPlayers();
    } catch (error) {
      console.warn('廣場角色同步失敗', error);
    }
  };
