(() => {
  ...
  function getLocalLook() {
    try {
      ...
      const eyeId=state?.game?.equipped?.eyeColor;const eyeItem=typeof itemById==='function'?itemById(eyeId):null;const skinId=state?.game?.skinColor||'ivory';return { gender, hairColor: colorItem?.color || '#9B6737', eyeColor:eyeItem?.color||'#6D462E', skinColor:skinId };
    } catch (_) {
      return { gender: 'male', hairColor: '#9B6737', eyeColor:'#6D462E', skinColor:'ivory' };
    }
  }
  ...
  function applyCanvasColors(players) {
    ...
    elements.forEach((el, index) => {
      ...
      if (canvas && player) {
        ...
        canvas.dataset.skinColor = player.skin_color || 'ivory';
      }
    });
