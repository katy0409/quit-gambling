...
let cloudClaimsDate='',cloudClaims=[];
function localClaimed(){try{return JSON.parse(localStorage.getItem('restart-daily-claimed-'+today())||'[]')}catch{return[]}}
function claimed(){return cloudClaimsDate===today()?cloudClaims:localClaimed()}
async function loadCloudClaims(){try{const {data:{user}}=await window.cloud.auth.getUser();if(!user)return;const {data,error}=await window.cloud.from('daily_task_claims').select('task_key').eq('user_id',user.id).eq('task_date',today());if(error)throw error;cloudClaimsDate=today();cloudClaims=(data||[]).map(x=>x.task_key);localStorage.setItem('restart-daily-claimed-'+today(),JSON.stringify(cloudClaims))}catch(e){console.warn('daily claims sync skipped',e)}}
function progress(){return TASKS.filter(t=>t.done()).length}
window.openDailyTasks=async function(){await loadCloudClaims();renderDailyTasks();openModal('dailyTasksModal')}
...
window.claimDailyTask=async function(id){const t=TASKS.find(x=>x.id===id);if(!t||!t.done())return;const c=claimed();if(c.includes(id))return;try{const {data:{user}}=await window.cloud.auth.getUser();if(user){const {data,error}=await window.cloud.rpc('claim_daily_task_v135',{task_key:id,reward_amount:t.reward});if(error)throw error;const row=Array.isArray(data)?data[0]:data;if(row?.tokens!=null)state.game.tokens=Number(row.tokens)}else state.game.tokens+=t.reward;c.push(id);cloudClaimsDate=today();cloudClaims=[...new Set(c)];localStorage.setItem('restart-daily-claimed-'+today(),JSON.stringify(cloudClaims));save();renderDailyTasks();renderGameHome();notify(`獲得 ${t.reward} 枚戒賭幣`)}catch(e){notify('領取失敗：'+e.message)}}
